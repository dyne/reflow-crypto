...
\begin{align*}
    ...
    \widehat{C}_w   &= c_m\cdot c + r_r\cdot G_1 + r_m\cdot HS = c_m\cdot (r\cdot G_1 +                     m\cdot HS) + r_r\cdot G_1 + r_m\cdot HS \\
...
different function calls: $keygen$ to create an ElGamal keypair and
$prepareBlindSign$ to generate a Coconut credential request. An
interactive exchange takes place between the Signer and the Issuer
that verifies the possession of the secret ElGamal key and signs a
credential to witness this condition.
