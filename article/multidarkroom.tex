...
\begin{itemize}
    ...
    \begin{itemize}
       ...
       \item[4.] the first layer of the signature $S \leftarrow r\cdot U$, later to be summed with all other signatures in a multi-party computation setup resulting in the final signature as $S \leftarrow r\cdot U + \sum_i sk_i\cdot U$
...
\begin{itemize}
    \item \textbf{Randomization phase.} Three new nonces $w_h, w_k, w_r\in \mathbb{Z}$ are generated, each one related to the input variables $m, k, r$ respectively as we will show soon; 
    ...
    \item \textbf{Response phase.} In order that the proof can be verified the protocol generates three more variables which are alleged inside the proof itself and link the nonces $w_h, w_k, w_r$ with $m, k, r$, i.e:
    \begin{align*}
        r_h &= w_h - c_h m \\
        r_k &= w_k - c_h k \\
        r_r &= w_r - c_h r
...
\begin{itemize}
    ...
    \begin{align*}
        ...
        &\phi'(h)=1\}
    \end{align*}
    with $\phi'$ being a predicate which is true on $m$, while $\zeta$ is simply the scalar multiplication $h\cdot U$.
\end{itemize}
