...
Reflow's unlinkability of credentials and signatures the privacy
...
Reflow can be adopted by such an application to remove the need of context-free identifiers and implement authentication functions through a disposable identity whose traceability is bound to a context UID. Furthermore, being a signature scheme, Reflow can add the feature of context-free verifiable signatures (and multi-signatures) that are untraceable in public, but can be traced and even revoked in a known context by the means of context-specific signature fingerprints.
...
This section lists results of evaluation benchmarks we have run using the Zencode scenario implementation of Multidarkroom on 6 target platforms, as a \textbf{Command line interface} (CLI), or a JavaScript-wrapped, Emscripten built \textbf{WASM library}:  
\begin{itemize}
  \item   \textbf{X86 64bit binary} CLI on an Intel(R) Core(TM) i5-5300U CPU @ 2.30GHz 
  \item   \textbf{ARM 32bit binary} CLI on a Raspberry Pi 4 board
  \item  \textbf{ARM 32bit binary} CLI on a Raspberry Pi 0 board
  \item  \textbf{WASM 32bit} library on an Intel(R) Core(TM) i5-5300U CPU @ 2.30GHz
  \item  \textbf{WASM 32bit} library on a Raspberry Pi 4 board
  \item  \textbf{WASM 32bit} library on a Raspberry Pi 0 board
\end{itemize}

The use case the CLI application is typically a server-side application or micro-service. The use case of the WASM library can be web application running in the browser, a server-side application running node JS or a mobile application. 
...
Our benchmarks provide lab measurements for the all the steps in the signature flow: for each of them is reported the time of execution (expressed in milliseconds) and the size of output (expressed in bytes) in different conditions with a progressive number of participants (5, 10, 50, 100, 1000 signatures).

The Raspberry 0 and 4 have been chosen as target because of their similarities to, respectively, very low-cost (5 USD) IoT devices and very low cost mobile phones (sub 100 USD).


\paragraph {Scripts description} \newline \linebreak

The scripts used in the flow and for bench-marking can be divided in three categories: 

\begin{itemize}
\item Scripts executed by anyone, marked with (A), include the creation of the Reflow seal (Session start), the aggregation of the signatures (Collect sign) and the verification of the signatures on Reflow seal. Their duration is proportional to the amount of participants and can vary greatly. 

\item Scripts executed by each participant, marked with (P), include participant's setup and signing, they're duration is not correlated with the amount of participants. 

\item Scripts executed by the issuer, marked (I), include issuer's setup and signing, they're duration is not correlated with the amount of participants.
\end{itemize}

Scripts executed 50 times and got the average, while OS running regularly


\paragraph {Considerations} \newline \linebreak

\begin{itemize}
\item in the (A) group of scripts, the duration grow linearly with the amount of participants. 

\item The benchmarks run on CLI have a comparable duration with their counterparts run on WASM, only when using an X86 machine. The respective durations differ greatly on ARM 32 based Raspberry Pi 0 and 4 machines. We consider this a signal of different level of optimization in the compiling of WASM for X86 and ARM 32.  

\item The main oddity in the benchmark is the duration of the issuer's keygen script on Raspberry Pi 0. Analyzing the reason of this value is beyond the scope of this paper. Nevertheless, since this script contains ECP2 pairing, our future research may include research aiming at optimizing the pairing algorythm or the compiling.   
\end{itemize}
    ...
    \caption{Execution timings on \textbf{CLI X64} in $seconds$ per $participant$}
      \label{tab:table1}
        \begin{tabular} {c|c|c|c|c|c|c}
          ...
          \midrule
          session\_start.zen & 0.0188 & 0.0253 & 0.0410 & 0.0623 & 0.1056 & 0.5039 \\
          collect\_sign.zen & 0.0729 & 0.0730 & 0.1031 & 0.1373 & 0.7435 & 3.6061 \\
          verify\_sign.zen & 0.0405 & 0.0515 & 0.0996 & 0.1619 & 1.3974 & 7.0460 \\
...
\begin{table}[h!]
  \begin{center}
    \caption{Execution timings on \textbf{CLI Raspberry 4} in $seconds$ per $participant$}
      \label{tab:table1}
        \begin{tabular} {c|c|c|c|c|c}
          \toprule
           \textbf{$_S / _P$} & \textbf{2} & \textbf{10} & \textbf{50} & \textbf{100} & \textbf{1000} \\
          \midrule
          session\_start.zen & 0.0516 & 0.0602 & 0.0674 & 0.0750 & 0.2762 \\
          collect\_sign.zen & 0.2087 & 0.2273 & 0.3031 & 0.3942 & 2.1849 \\
          verify\_sign.zen & 0.1049 & 0.1393 & 0.2978 & 0.4831 & 4.0688 \\
      \bottomrule % end of content
    \end{tabular}
  \end{center}
\end{table}
      ...
      \label{tab:table1}
        \begin{tabular} {c|c|c|c|c|c}
          \toprule
           \textbf{$_S / _P$} & \textbf{2} & \textbf{10} & \textbf{50} & \textbf{100} & \textbf{1000} \\
          \midrule
          session\_start.zen & 0.2760 & 0.2936 & 0.3413 & 0.3725 & 1.3097 \\
          collect\_sign.zen & 1.0439 & 1.1458 & 1.3688 & 1.7669 & 9.6403 \\
          verify\_sign.zen & 0.5753 & 0.7199 & 1.3381 & 2.1971 & 17.8956 \\
      \bottomrule % end of content
    \end{tabular}
  \end{center}
\end{table}
  ...
  \begin{center}
    \caption{Execution timings on \textbf{CLI} of scripts in $seconds$ per $platform$}
      \label{tab:table1}
        \begin{tabular} {c|c|c|c|c|c}
          \toprule
\textbf{$script / arch$} & \textbf{(P) Keygen} & \textbf{(P) Request} & \textbf{(P) Pub-Key} & \textbf{(P) Aggr. Cred.} & \textbf{(P) Sign Session} \\
          \midrule
			X64	&	0.0137	&	0.0391	&	0.0196	&	0.0219	&	0.0521		\\
      ...
      \label{tab:table1}
        \begin{tabular} {c|c|c|c|c|c}
          \toprule
           \textbf{$_S / _P$} & \textbf{2} & \textbf{10} & \textbf{50} & \textbf{100} & \textbf{1000} \\
          \midrule
          session\_start.zen & 0.0324 & 0.0312 & 0.0373 & 0.0516 & 0.1749 \\
          collect\_sign.zen & 0.1180 & 0.1092 & 0.1510 & 0.2467 & 1.1563 \\
          verify\_sign.zen & 0.0695 & 0.0688 & 0.1495 & 0.2641 & 2.1554 \\
      \bottomrule % end of content
    \end{tabular}
  \end{center}
\end{table}
      ...
      \label{tab:table1}
        \begin{tabular} {c|c|c|c|c|c}
          \toprule
           \textbf{$_S / _P$} & \textbf{2} & \textbf{10} & \textbf{50} & \textbf{100} & \textbf{1000} \\
          \midrule
          session\_start.zen & 0.1065 & 0.1092 & 0.1269 & 0.1459 & 0.5432 \\
          collect\_sign.zen & 0.3897 & 0.4170 & 0.5741 & 0.7654 & 4.3594 \\
          verify\_sign.zen & 0.1880 & 0.2510 & 0.5621 & 0.9494 & 8.1078 \\
      \bottomrule % end of content
    \end{tabular}
  \end{center}
\end{table}
      ...
      \label{tab:table1}
        \begin{tabular} {c|c|c|c|c|c}
          \toprule
           \textbf{$_S / _P$} & \textbf{2} & \textbf{10} & \textbf{50} & \textbf{100} & \textbf{1000} \\
          \midrule
          session\_start.zen & 0.8688 & 0.8711 & 1.0392 & 1.2257 & 4.7067 \\
          collect\_sign.zen & 2.7272 & 2.8665 & 3.9580 & 5.1851 & 29.0912 \\
          verify\_sign.zen & 1.3818 & 1.7820 & 3.8503 & 6.3604 & 53.8938 \\
      \bottomrule % end of content
    \end{tabular}
  \end{center}
\end{table}
...
\textbf{$script / arch$} & \textbf{(P) Keygen} & \textbf{(P) Request} & \textbf{(P) Pub-Key} & \textbf{(P) Aggr. Cred.} & \textbf{(P) Sign Session} \\
          \midrule
X64				&	0.02134	&	0.04906	&	0.02624	&	0.0304	&	0.074510204		\\
...
\textbf{$script / arch$} &  \textbf{(I) Keygen}& \textbf{(I) Pub-Key}& \textbf{(I) Sign Req.} \\
          \midrule
X64				&		0.020416667	&	0.03175	&	0.074688889	\\
